...
class Master(Script):
    ...
    def configure(self, env):
        ...
        File(format("{elastic_conf_dir}/elasticsearch.yml"),
             content=Template("elasticsearch.master.yml.j2",
                              configurations=configurations),
            owner=params.elastic_user,group=params.elastic_group
            )
	#cmd = format("cat /home/es/elasticsearch.master.yml.j2 > {elastic_conf_dir}/elasticsearch.yml")
	#Execute(cmd)
    ...
    def start(self, env):
        ...
        # Start Elasticsearch
	#cmd = format("mv /tmp/start.py {elastic_home_dir}")
